...
Validator.messageSource['en-us'] = [
	...
	['validate-alphameric','Please use only letters (a-z) and numbers (0-9) only in this field. No spaces or other characters are allowed.'],
	...
	['validate-one-required' , 'Please select one of the options.'],
	...
	['validate-test' , function(v,elm,args,metadata) {
		return ValidationUtils.format("Please enter file type [%s]",['xls']);
	}],
	//中国特有的相关验证提示信息
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-alphameric','请混合输入英文字母和数字,不要只输入英文字母或数字.'],
	['validate-email' , '请输入有效的邮件地址,如 username@example.com.'],
	['validate-url' , '请输入有效的URL地址.'],
	['validate-currency-dollar' , 'Please enter a valid $ amount. For example $100.00 .'],
	['validate-one-required' , '在选项中至少选择一个.'],
	...
	['validate-test' , function(v,elm,args,metadata) {
		return ValidationUtils.format("文件类型应该为[%s]",['xls']);
	}],
	//中国特有的相关验证提示信息
...
ValidationUtils = {
	...
	fireSubmit: function(form) {
 	...
 	getLanguage : function() {
 		var lang = getArgs()["hl"].toLowerCase();
 		if(lang != 'undefined'){
 			return lang;
		...
		if(this.options.immediate) {
			...
			for(var i = 0; i < elements.length; i++) {
				...
				Event.observe(input, 'blur', function(ev) {
					// TODO fixed:失去焦点时清楚提示信息
					var elm = Event.element(ev);
					var cn = $A(elm.classNames());
					for(var j = 0; j < cn.length; j++){
						Validation.hideErrorMsg(cn[j],elm);
					}
				 	Validation.validateElement(elm,{useTitle : useTitles, onElementValidate : callback}); 
				});
			}
		}
	},
...
Validation.addAllThese([
	...
	['validate-alphameric', function(v) {
				return !/\W/.test(v)
			}],
	['validate-one-required', function (v,elm) {
				//var p = elm.form;//TODO fixed,修复checkbox,radio外被标签包括bug
	...
	['validate-alphameric',/^([a-zA-z]+[0-9]+)|([0-9]+[a-zA-Z]+)[a-zA-Z0-9]*$/],	
