...
public class Nano implements Benchmark {
	...
	public void measure(Runner task) {
		measure("", task);
	}

	public void measure(String label, Runner task) {
		measure(label, (Runnable) task);
	}
	...
	private void doMeasure(String label, Runnable task) {
		...
		for (int i = 0; i < this.numberOfMeasurement; i++) {
			executor.execute(new TimeMeasureProxy(new MeasureState(label, i, this.numberOfMeasurement,
					this.numberOfThread), task, this.getListeners(), this.measureLatch));
		}
	...
	private void doWarmup(Runnable task) {
		...
		for (int i = 0; i < this.numberOfWarmUp; i++) {
			executor.execute(new TimeMeasureProxy(new MeasureState("_warmup_", i, this.numberOfWarmUp, 1), task, this
					.getListeners(), this.warmUpLatch));
		}
		try {
	...
	private static class TimeMeasureProxy implements Runnable {
		...
		public void run() {
			if (runnable instanceof Runner) {
				((Runner) runnable).beforeRun();// 预备数据
			}
			state.startNow();
			runnable.run();
			state.endNow();
